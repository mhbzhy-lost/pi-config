...
import { Container, Spacer, Text, sliceByColumn, truncateToWidth, visibleWidth, type MarkdownTheme } from "@earendil-works/pi-tui";
import { createCompactToolRenderers } from "../../../scripts/lib/compact-tools-renderer.mjs";

const compactToolRenderers = createCompactToolRenderers({ Text, Container, visibleWidth, sliceByColumn, truncateToWidth });
      ...
      switch (item.role) {
        ...
        case "assistant": {
          ...
          for (const content of item.content) {
            ...
            const tool = new ToolExecutionComponent(content.name, content.id, content.arguments, { showImages: false }, compactToolRenderers[content.name as keyof typeof compactToolRenderers], options.ui as any, options.cwd);
