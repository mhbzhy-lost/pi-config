...
export default function subagentRuntime(pi: ExtensionAPI): void {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const broker = new RootBrokerServer({ rootSessionId, lifecycleSessionId: ctx.currentSessionId, upstream, events: pi.events, recordRevivalDiagnostic: (customType, data) => pi.appendEntry(customType, data) });
