...
test("amendment crash restart Harness uses durable events for exact-once recovery", { timeout: 480000 }, async (t) => {
  ...
  } finally { try { await handlers?.get?.("session_shutdown")?.(); } catch {} try { if (second && host) await host.stop(second); } catch {} if (passed && process.env.PLAN_HARNESS_PRESERVE !== "1") await rm(tmp, { recursive: true, force: true, maxRetries: 5, retryDelay: 50 }); else t.diagnostic(`preserved=${tmp}`); }
});
