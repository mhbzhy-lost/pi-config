...
test("runtime approval pairs ABI-shaped input with the active branch's real user entry", async () => {
  ...
  const pending = JSON.parse(await invoke(api, "goal_status", {}));
  api.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: api.sessionManager });
  assert.equal(runtimeEntries(api, "decision").length, 0, "input hook records only intent");
  const intent = api.entries.find(entry => entry.customType === "goal-engine-runtime-approval-intent"); assert.ok(intent); assert.equal(Object.hasOwn(intent.data, "userEntryId"), false);
  const user = api.entries.at(-1); assert.equal(user.type, "message"); assert.equal(user.message.role, "user");
...
test("runtime approval fails closed for duplicate intents and non-idle or image input", async () => {
  for (const event of [
    { type: "input", source: "interactive", text: "approve", streamingBehavior: "steer" },
    { type: "input", source: "interactive", text: "approve", images: [{ type: "image", data: "x", mimeType: "image/png" }] },
  ]) {
    const cwd = repo(), api = pi(cwd); api.cwd = cwd; createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: host(cwd) }); await invoke(api, "goal_init", runtimeInit()); await invoke(api, "goal_status", {});
    api.handlers.get("input")(event, { cwd, sessionManager: api.sessionManager }); await invoke(api, "goal_status", {});
    assert.equal(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").runtimeState, "awaiting_user_approval");
  }
  const cwd = repo(), api = pi(cwd); api.cwd = cwd; createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: host(cwd) }); await invoke(api, "goal_init", runtimeInit()); await invoke(api, "goal_status", {});
  api.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: api.sessionManager });
  api.appendEntry("goal-engine-runtime-approval-intent", structuredClone(api.entries.find(entry => entry.customType === "goal-engine-runtime-approval-intent").data));
  await invoke(api, "goal_status", {});
  assert.equal(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").runtimeState, "awaiting_user_approval");
});
...
test("runtime authority is session-scoped: another session cannot discover or consume it", async () => {
  ...
  other.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: other.sessionManager });
...
test("challenge, approved decision, and consumed approval survive reload without duplicate nonce or event", async () => {
  ...
  challengeReload.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: challengeReload.sessionManager });
...
test("durable approval append throw is recovered from the real projection without replay", async () => {
  ...
  const api = pi(cwd); api.cwd = cwd; createGoalEngineExtension(api, options); await invoke(api, "goal_init", runtimeInit()); await invoke(api, "goal_status", {}); api.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: api.sessionManager });
...
test("runtime nonce is represented only by its digest in JSONL, Pi entries, and status", async () => {
  ...
  createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: { ...host(cwd), nonceFactory: () => raw } }); await invoke(api, "goal_init", runtimeInit()); await invoke(api, "goal_status", {}); api.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: api.sessionManager }); const status = await invoke(api, "goal_status", {});
...
async function prepareUserApprovedRepair(api, cwd) {
  await activateProduct(api, cwd, activeCondition());
  await cycleUntil(api, 1, "terminal");
  await invoke(api, "goal_status", {});
  await cycleUntil(api, 1, "released");
  await invoke(api, "goal_status", {});
  return [...loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").repairChallenges.values()][0];
}

test("Repair S1 challenge pre-append failure leaves no challenge and retry creates one", async () => {
  const cwd = repo(), api = pi(cwd); api.cwd = cwd; let failed = false;
  createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: observationHost(cwd, { codes: ["PASS", "FAIL"] }), appendEvent(root, event, version) {
    if (event.type === "repair.challenge_created" && !failed) { failed = true; throw Error("before challenge append"); }
  ...
  await assert.rejects(invoke(api, "goal_status", {}), /before challenge append/);
  assert.equal(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").repairChallenges.size, 0);
  assert.equal(JSON.parse(await invoke(api, "goal_status", {})).status, "R10A3_REPAIR_APPROVAL_REQUIRED");
  assert.equal(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").repairChallenges.size, 1);
});

test("Repair S1 challenge durable recovery rejects altered event data", async () => {
  const cwd = repo(), api = pi(cwd); api.cwd = cwd; let altered = false;
  createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: observationHost(cwd, { codes: ["PASS", "FAIL"] }), appendEvent(root, event, version) {
    if (event.type === "repair.challenge_created" && !altered) {
      altered = true; appendEvent(root, { ...event, data: { ...event.data, expiresAt: event.data.expiresAt - 1 } }, version); throw Error("after altered challenge append");
    }
    return appendEvent(root, event, version);
  } });
  await activateProduct(api, cwd, activeCondition()); await cycleUntil(api, 1, "terminal"); await invoke(api, "goal_status", {}); await cycleUntil(api, 1, "released");
  await assert.rejects(invoke(api, "goal_status", {}), /invalid repair challenge/);
});

test("Repair S1 challenge durable recovery reloads one exact challenge", async () => {
  const cwd = repo(), api = pi(cwd); api.cwd = cwd; let threw = false;
  const options = { goalStateEnv: {}, runtimeHost: observationHost(cwd, { codes: ["PASS", "FAIL"] }), appendEvent(root, event, version) {
    const result = appendEvent(root, event, version); if (event.type === "repair.challenge_created" && !threw) { threw = true; throw Error("after challenge append"); } return result;
  } };
  createGoalEngineExtension(api, options); const challenge = await prepareUserApprovedRepair(api, cwd);
  const reloaded = pi(cwd, api.entries); reloaded.cwd = cwd; createGoalEngineExtension(reloaded, options); reloaded.handlers.get("session_start")({}, { sessionManager: reloaded.sessionManager });
  const result = JSON.parse(await invoke(reloaded, "goal_status", {}));
  assert.equal(result.challenge.challengeId, challenge.challengeId); assert.equal(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").repairChallenges.size, 1);
});

test("Repair S2 durable decision recovery reloads the exact approved decision once", async () => {
  const cwd = repo(), api = pi(cwd); api.cwd = cwd; let threw = false;
  const options = { goalStateEnv: {}, runtimeHost: observationHost(cwd, { codes: ["PASS", "FAIL"] }), appendEvent(root, event, version) {
    const result = appendEvent(root, event, version); if (event.type === "repair.user_decision_recorded" && !threw) { threw = true; throw Error("after repair decision append"); } return result;
  } };
  createGoalEngineExtension(api, options); const challenge = await prepareUserApprovedRepair(api, cwd);
  api.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: api.sessionManager });
  assert.equal(JSON.parse(await invoke(api, "goal_status", {})).status, "R10A3_REPAIR_APPROVAL_RECORDED");
  const rows = observationEventRows(cwd).filter(event => event.type === "repair.user_decision_recorded"); assert.equal(rows.length, 1);
  const decision = rows[0].data, approved = loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").repairChallenges.get(challenge.challengeId);
  assert.equal(approved.decisionId, decision.decisionId); assert.equal(approved.branchBindingHash, decision.branchBindingHash);
  const reloaded = pi(cwd, api.entries); reloaded.cwd = cwd; createGoalEngineExtension(reloaded, options); reloaded.handlers.get("session_start")({}, { sessionManager: reloaded.sessionManager });
  assert.equal(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").repairChallenges.get(challenge.challengeId).decisionId, decision.decisionId);
...
test("Repair S2 accepts one strict Pi compaction on the active branch", async () => {
  ...
  const intent = api.entries.find(entry => entry.customType === "goal-engine-repair-approval-intent"), user = api.entries.at(-1), start = Date.now() + 1_000;
  intent.timestamp = new Date(start).toISOString(); user.timestamp = new Date(start + 2).toISOString(); const compact = { id: "repair-strict-compact", parentId: intent.id, timestamp: new Date(start + 1).toISOString(), type: "compaction", summary: "Pi summary", firstKeptEntryId: "kept", tokensBefore: 1 };
  user.parentId = compact.id; api.entries.splice(api.entries.indexOf(user), 0, compact);
  assert.equal(JSON.parse(await invoke(api, "goal_status", {})).status, "R10A3_REPAIR_APPROVAL_RECORDED");
  assert.equal(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").repairChallenges.get(challenge.challengeId).phase, "approved");
});

test("Repair S2 rejects an off-branch approval message even when getEntries contains it", async () => {
  const cwd = repo(), api = pi(cwd); api.cwd = cwd; createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: observationHost(cwd, { codes: ["PASS", "FAIL"] }) });
  const challenge = await prepareUserApprovedRepair(api, cwd); api.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: api.sessionManager });
  const reloaded = pi(cwd, api.entries, { branchEntries: api.entries.slice(0, api.entries.findIndex(entry => entry.customType === "goal-engine-repair-approval-intent")) }); reloaded.cwd = cwd;
  createGoalEngineExtension(reloaded, { goalStateEnv: {}, runtimeHost: observationHost(cwd, { codes: ["PASS", "FAIL"] }) }); reloaded.handlers.get("session_start")({}, { sessionManager: reloaded.sessionManager });
  assert.equal(JSON.parse(await invoke(reloaded, "goal_status", {})).status, "R10A3_REPAIR_APPROVAL_REQUIRED");
  assert.equal(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").repairChallenges.get(challenge.challengeId).phase, "created");
});

test("Repair S2 reject records only rejected phase without Task or nonce", async () => {
  const cwd = repo(), api = pi(cwd); api.cwd = cwd; createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: observationHost(cwd, { codes: ["PASS", "FAIL"] }) });
  const challenge = await prepareUserApprovedRepair(api, cwd); api.handlers.get("input")({ type: "input", source: "interactive", text: "reject" }, { cwd, sessionManager: api.sessionManager });
  const result = JSON.parse(await invoke(api, "goal_status", {})), projection = loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime");
  assert.equal(result.status, "R10A3_REPAIR_APPROVAL_REJECTED"); assert.equal(projection.repairChallenges.get(challenge.challengeId).phase, "rejected"); assert.equal(projection.tasks.size, 0); assert.equal(JSON.stringify(projection).includes("nonce"), false);
  assert.equal(api.entries.some(entry => entry.customType === "goal-engine-repair-episode-rejected-by-user"), false);
});

test("Repair S2 audit receipt failure does not revoke the Goal decision", async () => {
  const cwd = repo(), api = pi(cwd); let failAudit = true; api.cwd = cwd;
  const append = api.appendEntry; api.appendEntry = (type, data) => { if (type === "goal-engine-repair-approval-decision" && failAudit) { failAudit = false; throw Error("audit unavailable"); } return append(type, data); };
  createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: observationHost(cwd, { codes: ["PASS", "FAIL"] }) }); const challenge = await prepareUserApprovedRepair(api, cwd);
  api.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: api.sessionManager });
  assert.equal(JSON.parse(await invoke(api, "goal_status", {})).status, "R10A3_REPAIR_APPROVAL_RECORDED");
  assert.equal(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").repairChallenges.get(challenge.challengeId).phase, "approved");
});
...
test("Repair S3 pre-append failure preserves approved authority and retries one canonical batch", async () => {
  ...
  first.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: first.sessionManager }); await invoke(first, "goal_status", {});
  let failed = false; const injected = injectedStore(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime"), entries => !failed && entries.map(event => event.type).join(",") === "goal.amended,repair.capability_consumed,repair.task_linked" ? (failed = true, "pre") : null);
  const api = pi(cwd, first.entries); api.cwd = cwd; createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: durable, store: injected.store });
  await assert.rejects(invoke(api, "goal_status", {}), /injected pre-append failure/);
  let projection = injected.latest(); assert.equal(projection.repairChallenges.get(challenge.challengeId).phase, "approved"); assert.equal(projection.tasks.size, 0); assert.equal(injected.events.filter(event => event.type !== "goal.checkpoint").length, 0);
  const retried = JSON.parse(await invoke(api, "goal_status", {})); projection = injected.latest(); assert.equal(retried.status, "R10A3_REPAIR_MATERIALIZED"); assert.deepEqual(injected.batches.at(-1).map(event => event.type), ["goal.amended", "repair.capability_consumed", "repair.task_linked"]); assert.equal(projection.tasks.size, 1);
});

test("Repair S3 durable throw exactly recovers consume Task and Episode link before reload dispatch", async () => {
  const cwd = repo(), first = pi(cwd); first.cwd = cwd; let calls = 0; const durable = { ...observationHost(cwd, { codes: ["PASS", "FAIL"] }), issueRepairCapability(input) { calls++; return issueRepairCapability(input); } };
  createGoalEngineExtension(first, { goalStateEnv: {}, runtimeHost: durable }); const challenge = await prepareUserApprovedRepair(first, cwd); first.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: first.sessionManager }); await invoke(first, "goal_status", {});
  let thrown = false; const injected = injectedStore(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime"), entries => !thrown && entries[1]?.type === "repair.capability_consumed" ? (thrown = true, "durable") : null);
  const api = pi(cwd, first.entries); api.cwd = cwd; createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: durable, store: injected.store });
  assert.equal(JSON.parse(await invoke(api, "goal_status", {})).status, "R10A3_REPAIR_MATERIALIZED"); const recovered = injected.latest(), taskId = challenge.taskId;
  assert.equal(recovered.repairChallenges.get(challenge.challengeId).phase, "applied"); assert.equal(recovered.tasks.get(taskId).metadata.episodeId, challenge.episodeId); assert.deepEqual(recovered.repairEpisodes.get(challenge.episodeId).remediationTaskIds, [taskId]); assert.equal(calls, 1);
  const reload = pi(cwd, api.entries); reload.cwd = cwd; createGoalEngineExtension(reload, { goalStateEnv: {}, runtimeHost: durable, store: injected.store }); const offer = JSON.parse(await invoke(reload, "goal_status", {})); assert.equal(offer.machineAction?.tool, "goal_dispatch"); assert.equal(calls, 1); assert.equal(injected.events.filter(event => event.type === "repair.capability_consumed").length, 1);
});

test("Repair S3 rejects durable recovery when consumedAt differs despite the same nonce digest", async () => {
  const cwd = repo(), first = pi(cwd); first.cwd = cwd; const durable = observationHost(cwd, { codes: ["PASS", "FAIL"] }); createGoalEngineExtension(first, { goalStateEnv: {}, runtimeHost: durable }); await prepareUserApprovedRepair(first, cwd); first.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: first.sessionManager }); await invoke(first, "goal_status", {});
  let current = structuredClone(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime")), done = false; const store = { listGoals: () => [current.goalId], listGoalIds: () => [current.goalId], loadProjection: () => current, appendEvent(_root, event, version) { assert.equal(version, current.version); current = applyEvent(current, event); return current; }, appendEventBatch(_root, events, version) { assert.equal(version, current.version); for (const event of events) current = applyEvent(current, event.type === "repair.capability_consumed" ? { ...event, data: { ...event.data, consumedAt: event.data.consumedAt + 1 } } : event); if (!done) { done = true; throw Error("altered durable S3"); } return current; } };
  const api = pi(cwd, first.entries); api.cwd = cwd; createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: durable, store }); await assert.rejects(invoke(api, "goal_status", {}), /altered durable S3/);
});

test("Repair S3 rejects durable recovery when canonical Task metadata drifts", async () => {
  const cwd = repo(), first = pi(cwd); first.cwd = cwd; const durable = observationHost(cwd, { codes: ["PASS", "FAIL"] }); createGoalEngineExtension(first, { goalStateEnv: {}, runtimeHost: durable }); await prepareUserApprovedRepair(first, cwd); first.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: first.sessionManager }); await invoke(first, "goal_status", {});
  let current = structuredClone(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime")); const store = { listGoals: () => [current.goalId], listGoalIds: () => [current.goalId], loadProjection: () => current, appendEvent(_root, event, version) { assert.equal(version, current.version); current = applyEvent(current, event); return current; }, appendEventBatch(_root, events, version) { assert.equal(version, current.version); for (const event of events) current = applyEvent(current, event); const [task] = current.tasks.values(); task.metadata.identity = "drifted"; throw Error("drifted durable S3"); } };
...
test("Repair S3 returns expired without issuer or batch", async () => {
  const cwd = repo(), api = pi(cwd); api.cwd = cwd; let now = Date.now(), issuerCalls = 0, batches = 0; const durable = { ...observationHost(cwd, { codes: ["PASS", "FAIL"] }), clock: () => now, issueRepairCapability(input) { issuerCalls++; return issueRepairCapability(input); } };
  createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: durable, appendEventBatch(...args) { batches++; return appendEventBatch(...args); } }); await prepareUserApprovedRepair(api, cwd); api.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: api.sessionManager }); await invoke(api, "goal_status", {}); const challenge = [...loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime").repairChallenges.values()][0]; now = challenge.expiresAt; batches = 0; assert.equal(JSON.parse(await invoke(api, "goal_status", {})).status, "R10A3_REPAIR_APPROVAL_EXPIRED"); assert.equal(issuerCalls, 0); assert.equal(batches, 0);
});

test("Repair S3 treats consumed pending authority as ambiguous and never falls through", async () => {
  const cwd = repo(), first = pi(cwd); first.cwd = cwd; const durable = observationHost(cwd, { codes: ["PASS", "FAIL"] }); createGoalEngineExtension(first, { goalStateEnv: {}, runtimeHost: durable }); const challenge = await prepareUserApprovedRepair(first, cwd); first.handlers.get("input")({ type: "input", source: "interactive", text: "approve" }, { cwd, sessionManager: first.sessionManager }); await invoke(first, "goal_status", {});
  const injected = injectedStore(loadProjection(join(cwd, ".state/goal-engine"), "harden-runtime")); injected.latest().repairChallenges.get(challenge.challengeId).phase = "consumed"; const api = pi(cwd, first.entries); api.cwd = cwd; createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: durable, store: injected.store }); const status = JSON.parse(await invoke(api, "goal_status", {})); assert.equal(status.attention?.[0], "R10A3_REPAIR_APPROVAL_AMBIGUOUS"); assert.equal(injected.events.filter(event => event.type !== "goal.checkpoint").length, 0);
});
