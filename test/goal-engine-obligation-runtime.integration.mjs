...
function pi(cwd) { const tools = [], entries = [], manager = { getSessionId: () => "owner", getSessionFile: () => join(cwd, "session"), getLeafId: () => "leaf", getEntries: () => entries }; return { tools, entries, sessionManager: manager, registerTool: tool => tools.push(tool), on() {}, appendEntry: (customType, data) => entries.push({ type: "custom", customType, data }) }; }
