...
async function suspended({ batch } = {}) { const cwd = repo(), api = pi(cwd); createGoalEngineExtension(api, { goalStateEnv: {}, runtimeHost: host(cwd), ...(batch ? { appendEventBatch: batch } : {}) }); await invoke(api, "goal_init", runtimeInit()); await invoke(api, "goal_status"); await api.handlers.get("input")({ type: "input", text: "approve", source: "interactive" }, { cwd, sessionManager: api.sessionManager }); for (let i = 0; i < 30 && projectionFor(cwd).runtimeState !== "active"; i++) await invoke(api, "goal_status"); assert.equal(projectionFor(cwd).runtimeState, "active"); const initial = { suspensionId: "resume-closure", reason: "host_pause", affectedTaskIds: [], affectedRunIds: [], requestedAt: "2026-08-20T00:00:00.000Z", resourcesQuarantined: false }; let projection = projectionFor(cwd); appendEvent(rootFor(cwd), event(projection.goalId, "goal.runtime_suspended", initial), projection.version); const closure = { ...initial, resourcesQuarantined: true, terminalProofRefs: [], workspaceClosureProofRefs: [], resourceClosureProofRefs: [] }; projection = projectionFor(cwd); appendEvent(rootFor(cwd), event(projection.goalId, "goal.runtime_suspended", closure), projection.version); return { cwd, api, closure }; }
