...
test("amendment enters suspended through its durable runtime event", () => {
  ...
  const approvalMaterial = { proposalId: "p", proposalHash: proposal.proposalHash, ownerSessionId: "session", userEntryId: "u", userEntryHash: hash(5), branchBindingHash: hash(6), choice: "approve", approved: true, source: "interactive", recordedAt: "2026-08-20T00:00:17.000Z" };
...
test("runtime store replay persists repair challenge after complete calibration, activation, and product failure", () => {
  ...
  try {
    ...
    const challenge = createRepairChallenge({ projection: p, episodeId: "episode-1", action: "reject", sessionId: "session-1", requestedAt: 20, expiresAt: 30, baseHead: "b".repeat(40), subjectHash: rejectSubjectHash(p, p.repairEpisodes.get("episode-1")) });
    p = appendEvent(root, event(challenge.events[0].type, challenge.events[0].data, 17), p.version); const decision = recordRepairUserDecision({ projection: p, challengeId: challenge.challengeId, sessionId: "session-1", userEntryId: "entry-1", userEntryHash: hash(501), branchBindingHash: hash(502), userEntryOccurredAt: 21, choice: "approve", approved: true, source: "interactive", recordedAt: 21 });
