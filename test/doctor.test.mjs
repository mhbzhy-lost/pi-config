...
test("inspectConfiguration accepts a configured pi-subagents package", async () => {
  ...
  try {
    ...
    for (const skill of ["external-llm-review", "git-commit-convention", "systematic-debugging", "test-driven-development", "receiving-code-review", "writing-skills", "writing-plans", "plan-runner-dispatch"]) {
      await mkdir(join(root, "skill-overrides", skill), { recursive: true });
      await writeFile(join(root, "skill-overrides", skill, "SKILL.md"), "# test\n");
    }
    await writeFile(
      join(root, "agents", "skills.list"),
    ...
    assert.deepEqual(await inspectConfiguration(root, { readPiVersion: async () => "0.80.6" }), []);
  } finally {
...
test("inspectConfiguration requires the Parent-owned Plan lifecycle helper", async () => {
  ...
  try {
    await mkdir(join(root, "agents"), { recursive: true });
    await writeFile(join(root, "agents", "skills.list"), "");
    await mkdir(join(root, "pi", "extensions"), { recursive: true });
    await mkdir(join(root, "scripts"), { recursive: true });
    ...
    const issues = await inspectConfiguration(root, { readPiVersion: async () => "0.80.6" });
...
test("inspectConfiguration reports the actual Pi executable version", async () => {
  const issues = await inspectConfiguration(repoRoot, { readPiVersion: async () => "0.80.5" });
  assert.ok(issues.includes("unexpected Pi version: 0.80.5; expected 0.80.6"));
});
...
test("inspectConfiguration reports the final plan execution contract gaps", async () => {
  ...
  try {
    await mkdir(join(root, "agents"), { recursive: true });
    await writeFile(join(root, "agents", "skills.list"), "external-llm-review\n");
    ...
    const issues = await inspectConfiguration(root, { readPiVersion: async () => "unknown" });
    assert.ok(issues.includes("unexpected Pi version: unknown; expected 0.80.6"));
