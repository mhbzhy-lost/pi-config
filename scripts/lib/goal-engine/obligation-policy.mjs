...
export function actionableFrontier(input = {}) {
  ...
  const reverifyingEpisode = values(projection.repairEpisodes ?? {}, "repairEpisodes").some(episode => episode?.status === "reverifying");
  for (const [id, task] of collection(projection.tasks ?? {}, "tasks")) {
    ...
    if (active && next?.tool === "goal_settle") note(blocking, "task", id, "TASK_FUTURE_WAKE"); else if (next?.tool === "goal_accept" && reverifyingEpisode) note(blocking, "task", id, "REPAIR_REVERIFICATION_PENDING"); else if (next) action(actions, "task", id, next.priority, next.tool, next.params, next.reason); else if (["dispatched", "running", "settling"].includes(task?.status)) note(blocking, "task", id, "TASK_FUTURE_WAKE");
  }
