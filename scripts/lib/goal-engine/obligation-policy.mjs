...
const TERMINAL_RUN_STATES = new Set(["terminal", "released", "cancelled", "failed", "succeeded"]);
...
function progressAuthority(projection, blocking, attention) {
  ...
  if (ledger.some(row => !isObject(row) || Object.keys(row).length !== 3 || !["canonicalFingerprint", "advanced", "sequence"].every(key => Object.hasOwn(row, key)) || !/^[a-f0-9]{64}$/.test(row.canonicalFingerprint) || typeof row.advanced !== "boolean" || !Number.isSafeInteger(row.sequence) || row.sequence <= last || seen.has(row.canonicalFingerprint) || !(seen.add(row.canonicalFingerprint)) || !(last = row.sequence))) { note(attention, "budget", "no-progress", "NO_PROGRESS_AUTHORITY_UNAVAILABLE"); return false; }
...
export function actionableFrontier(input = {}) {
  ...
  for (const [key, run] of collection(projection.observationRuns ?? {}, "observationRuns")) {
    ...
    else if (ACTIVE_RUN_PHASES.has(run.phase)) note(blocking, "observation", id, "OBSERVATION_FUTURE_WAKE");
    else if (run.phase !== "released") note(attention, "observation", id, "OBSERVATION_PHASE_UNKNOWN");
  }
  const activeWorldRuns = Array.isArray(world.activeRuns) ? world.activeRuns : null;
  if (!activeWorldRuns) note(attention, "world", "active-runs", "ACTIVE_RUN_AUTHORITY_UNAVAILABLE");
  else {
    for (const [key, run] of collection(projection.observationRuns ?? {}, "observationRuns")) {
      const id = run?.runId ?? key, active = activeWorldRuns.some(candidate => candidate?.runId === id && !TERMINAL_RUN_STATES.has(candidate?.state));
      if ((ACTIVE_RUN_PHASES.has(run?.phase) && !active) || (["terminal", "recorded", "released"].includes(run?.phase) && active)) note(attention, "observation", String(id), "OBSERVATION_RUN_STATE_CONFLICT");
    }
    if (activeWorldRuns.some(run => !TERMINAL_RUN_STATES.has(run?.state))) note(blocking, "world", "active-runs", "OWNED_ACTIVE_RUN");
  }
  for (const [id, task] of collection(projection.tasks ?? {}, "tasks")) {
    ...
    const bound = task?.executorBinding?.runId; const active = validId(bound) && activeWorldRuns?.some(run => run?.runId === bound && !TERMINAL_RUN_STATES.has(run?.state));
  ...
  for (const [id, state] of collection(projection.conditions ?? {}, "conditions")) {
    ...
    const deps = dependencyReady(state, projection, blocking, id); if (["satisfied", "stale", "blocked"].includes(state.status)) { if (state.status === "blocked") note(blocking, "condition", id, "CONDITION_BLOCKED"); continue; }
    ...
    if (!deps) continue; const claims = claimsFor(id, inventory); if (!claims) { note(attention, "condition", id, "RESOURCE_CLAIM_AUTHORITY_UNAVAILABLE"); continue; } if (resourceBlocked(claims, world.resources)) { note(blocking, "condition", id, "RESOURCE_CONFLICT"); continue; } action(actions, "condition", id, 5, "request_observation", { condition_id: id }, "Condition requires fresh observation");
  }
  budgetFacts(projection, world, blocking, attention);
  const globalGate = blocking.some(item => ["WORLD_SNAPSHOT_UNSAFE", "OBSERVATION_BUDGET_EXHAUSTED", "REPAIR_BUDGET_EXHAUSTED", "ELAPSED_BUDGET_EXHAUSTED", "NO_PROGRESS_BUDGET_EXHAUSTED", "PENDING_HUMAN_DECISION"].includes(item.code)) || attention.some(item => ["NO_PROGRESS_AUTHORITY_UNAVAILABLE", "OBSERVATION_RUN_STATE_CONFLICT"].includes(item.code));
  const permitted = globalGate ? actions.filter(item => ["suspension-recovery", "resource-recovery", "observation-record", "observation-release", "task"].includes(item.kind) && item.priority <= 3) : actions;
