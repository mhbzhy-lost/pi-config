...
const challengeBinding = (c) => ({ challengeId: c.challengeId, goalId: c.goalId, executionRevision: c.executionRevision, executionContractHash: c.executionContractHash, baseHead: c.baseHead, episodeId: c.episodeId, conditionId: c.conditionId, findingIds: [...c.findingIds], action: c.action, subjectHash: c.subjectHash, taskId: c.taskId, taskDefHash: c.taskDefHash, sessionId: c.sessionId, challengeHash: c.challengeHash });
const publicBinding = (c) => ({ challengeId: c.challengeId, challengeHash: c.challengeHash, episodeId: c.episodeId, action: c.action, subjectHash: c.subjectHash, sessionId: c.sessionId, userEntryId: c.userEntryId, decisionId: c.decisionId, executionRevision: c.executionRevision, executionContractHash: c.executionContractHash, baseHead: c.baseHead, taskId: c.taskId, taskDefHash: c.taskDefHash, userEntryHash: c.userEntryHash, branchBindingHash: c.branchBindingHash });
...
function validateCapability(capability, p, episode, action, subjectHash, consumedAt) {
  const fields = ["prefix", "goalId", "executionRevision", "challengeId", "challengeHash", "episodeId", "action", "subjectHash", "sessionId", "userEntryId", "decisionId", "executionContractHash", "baseHead", "taskId", "taskDefHash", "userEntryHash", "branchBindingHash", "nonce", "singleUse"];
  if (!Number.isFinite(consumedAt) || !exact(capability, fields) || capability.prefix !== "goal-repair-capability.v1" || capability.goalId !== p.goalId || capability.executionRevision !== p.executionRevision || capability.episodeId !== episode?.episodeId || capability.action !== action || capability.subjectHash !== subjectHash || capability.singleUse !== true || typeof capability.nonce !== "string" || !capability.nonce) fail("durable challenge-bound capability required");
...
export function issueRepairCapability({ projection, challengeId, taskDef, now } = {}) {
  ...
  // A separately approved legacy episode-reject action remains distinct from a
...
export function buildRemediationTaskCandidate({ projection, episodeId, findingIds, taskDef } = {}) {
  ...
  const normalizedTaskDef = { ...structuredClone(taskDef), deps: taskDef.deps === undefined ? [] : structuredClone(taskDef.deps) };
  validateTaskDefinitions([taskId], { [taskId]: normalizedTaskDef }, { planned: true });
  if (normalizedTaskDef.workflow !== "tdd" || !Array.isArray(projection.writePolicy?.allowedPaths) || !Array.isArray(remediation.allowed_paths) || !normalizedTaskDef.writePaths.every((path) => pathCovered(path, projection.writePolicy.allowedPaths) && pathCovered(path, remediation.allowed_paths))) fail("write paths are not a provable subset of repair scope");
...
export function createRepairChallenge({ projection, episodeId, action, sessionId, requestedAt, expiresAt, baseHead, subjectHash, taskId = null, taskDefHash = null, taskDef } = {}) {
  ...
  const body = { goalId: projection.goalId, executionRevision: projection.executionRevision, executionContractHash: projection.executionContractHash, baseHead, episodeId, conditionId: episode.conditionId, findingIds, action, subjectHash, taskId, taskDefHash, sessionId, requestedAt, expiresAt };
