...
const publicBinding = (c) => ({ challengeId: c.challengeId, episodeId: c.episodeId, action: c.action, subjectHash: c.subjectHash, sessionId: c.sessionId, userEntryId: c.userEntryId, decisionId: c.decisionId, executionRevision: c.executionRevision, executionContractHash: c.executionContractHash, baseHead: c.baseHead, taskId: c.taskId, taskDefHash: c.taskDefHash, userEntryHash: c.userEntryHash, branchBindingHash: c.branchBindingHash });
...
export function recordRepairUserDecision({ projection, challengeId, sessionId, userEntryId, userEntryHash, branchBindingHash, userEntryOccurredAt, choice, approved, source, recordedAt } = {}) {
  ...
  const decisionId = digest({ challengeId, sessionId, userEntryId, userEntryHash, branchBindingHash, choice, approved, source, userEntryOccurredAt, recordedAt });
  return Object.freeze({ events: Object.freeze([event("repair.user_decision_recorded", { challengeId, sessionId, userEntryId, userEntryHash, branchBindingHash, userEntryOccurredAt, choice, approved, source, recordedAt, decisionId })]) });
}
