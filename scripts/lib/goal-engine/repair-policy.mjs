...
export function recordRepairUserDecision({ projection, challengeId, sessionId, userEntryId, userEntryHash, branchBindingHash, userEntryOccurredAt, choice, approved, source, recordedAt } = {}) {
  ...
  if (!c || c.phase !== "created" || c.sessionId !== sessionId || !Number.isFinite(userEntryOccurredAt) || !Number.isFinite(recordedAt) || !(c.requestedAt < userEntryOccurredAt && userEntryOccurredAt <= recordedAt && recordedAt < c.expiresAt) || !ID.test(userEntryId || "") || !HASH.test(userEntryHash || "") || !HASH.test(branchBindingHash || "") || !["approve", "reject"].includes(choice) || approved !== parity || !["interactive", "rpc"].includes(source)) fail("invalid real user decision");
