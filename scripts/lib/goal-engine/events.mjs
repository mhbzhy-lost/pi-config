...
function repairChallengeCreated(p, d) { runtimeOnly(p); requireExactFields(d, ["challengeId", "executionRevision", "episodeId", "action", "subjectHash", "sessionId", "requestedAt", "expiresAt", "nonceDigest"], "repair challenge"); if (!p.repairEpisodes.has(d.episodeId) || p.repairChallenges.has(d.challengeId) || d.executionRevision !== p.executionRevision || !["authorize_task", "reject"].includes(d.action) || !hash(d.subjectHash) || !d.sessionId || !Number.isFinite(d.requestedAt) || !Number.isFinite(d.expiresAt) || d.expiresAt <= d.requestedAt || !hash(d.nonceDigest)) throw new Error("invalid repair challenge"); p.repairChallenges.set(d.challengeId, { ...structuredClone(d), phase: "created" }); }
function repairUserDecisionRecorded(p, d) { runtimeOnly(p); requireExactFields(d, ["challengeId", "sessionId", "userEntryId", "approved", "source"], "repair user decision"); const c = p.repairChallenges.get(d.challengeId); if (!c || c.phase !== "created" || c.sessionId !== d.sessionId || c.expiresAt <= Date.now() || !d.userEntryId || typeof d.approved !== "boolean" || !["interactive", "rpc"].includes(d.source)) throw new Error("invalid repair user decision"); c.phase = d.approved ? "approved" : "rejected"; c.userEntryId = d.userEntryId; }
function repairCapabilityConsumed(p, d) { runtimeOnly(p); requireExactFields(d, ["challengeId", "nonceDigest"], "repair capability consume"); const c = p.repairChallenges.get(d.challengeId); if (!c || c.phase !== "approved" || c.nonceDigest !== d.nonceDigest) throw new Error("invalid repair capability consume"); c.phase = "consumed"; }
function repairRejectedByUser(p, d) { runtimeOnly(p); requireExactFields(d, ["episodeId", "conditionId", "findingIds", "reasonCode"], "repair rejection"); const e = p.repairEpisodes.get(d.episodeId); if (!e || e.conditionId !== d.conditionId || !["active", "waiting_for_tasks", "reverifying"].includes(e.status) || d.reasonCode !== "repair_rejected" || d.findingIds.length !== e.findingIds.length || d.findingIds.some((id) => !e.findingIds.includes(id))) throw new Error("invalid repair rejection"); e.status = "resolved"; for (const id of e.findingIds) p.findings.get(id).status = "rejected_by_user"; }
