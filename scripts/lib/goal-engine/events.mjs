...
function observationRequested(p, data) { runtimeOnly(p); requireExactFields(data, ["runId", "conditionId", "cycle", "worldSnapshotHash", "resourceClaimsHash"], "observation request"); if (!p.conditions.has(data.conditionId) || p.observationRuns.has(data.runId) || !Number.isSafeInteger(data.cycle) || data.cycle < 0 || !hash(data.worldSnapshotHash) || !hash(data.resourceClaimsHash) || (p.runtimeState === "calibrating" ? data.cycle !== 0 : p.runtimeState === "active" ? data.cycle < 1 : true)) throw new Error("invalid observation request"); const run = { runId: data.runId, conditionId: data.conditionId, cycle: data.cycle, worldSnapshotHash: data.worldSnapshotHash, resourceClaimsHash: data.resourceClaimsHash, phase: "requested", allocationId: null, leaseReceiptHash: null, processIdentityHash: null, terminalProofHash: null, evidenceId: null, releaseReceiptHash: null }; p.observationRuns.set(data.runId, run); const condition = p.conditions.get(data.conditionId); condition.status = "observing"; condition.lastObservationRunId = data.runId; }
