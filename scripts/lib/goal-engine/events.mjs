...
function runtimeActivated(p) { runtimeOnly(p); if (p.runtimeState !== "calibrating") throw new Error("runtime activation is out of order"); for (const [conditionId, condition] of p.conditions) { const run = [...p.observationRuns.values()].find((value) => value.conditionId === conditionId && value.cycle === 0 && ["recorded", "released"].includes(value.phase) && value.evidenceId); const evidence = run && p.evidenceHistory.find((value) => value.evidenceId === run.evidenceId && value.executionRevision === p.executionRevision); if (!evidence || !["passed", "failed"].includes(evidence.verdict?.kind)) throw new Error("runtime activation requires decidable cycle zero calibration"); condition.status = "inactive"; condition.supportingEvidenceIds = []; } p.runtimeState = "active"; }
...
function observationRequested(p, data) { runtimeOnly(p); requireExactFields(data, ["runId", "conditionId", "cycle", "worldSnapshotHash", "resourceClaimsHash"], "observation request"); if (!p.conditions.has(data.conditionId) || p.observationRuns.has(data.runId) || !Number.isSafeInteger(data.cycle) || data.cycle < 0 || (p.runtimeState === "calibrating" ? data.cycle !== 0 : p.runtimeState === "active" ? data.cycle < 1 : true)) throw new Error("invalid observation request"); const run = { runId: data.runId, conditionId: data.conditionId, cycle: data.cycle, phase: "requested", allocationId: null, leaseReceiptHash: null, processIdentityHash: null, terminalProofHash: null, evidenceId: null, releaseReceiptHash: null }; p.observationRuns.set(data.runId, run); const condition = p.conditions.get(data.conditionId); condition.status = "observing"; condition.lastObservationRunId = data.runId; }
function observationTransition(p, data, from, to) { runtimeOnly(p); const run = p.observationRuns.get(data.runId); if (!run || run.conditionId !== data.conditionId || run.phase !== from) throw new Error("invalid observation phase"); if (to === "lease_allocated" && (typeof data.allocationId !== "string" || !data.allocationId)) throw new Error("invalid observation allocation"); if (to === "process_bound" && !data.processIdentityHash) throw new Error("invalid observation process identity"); if (to === "terminal" && !data.terminalProofHash) throw new Error("invalid observation terminal proof"); if (to === "released" && !hash(data.releaseReceiptHash)) throw new Error("invalid observation release proof"); Object.assign(run, { phase: to, ...(to === "lease_allocated" ? { allocationId: data.allocationId, leaseReceiptHash: data.leaseReceiptHash || null } : {}), ...(to === "process_bound" ? { processIdentityHash: data.processIdentityHash } : {}), ...(to === "terminal" ? { terminalProofHash: data.terminalProofHash } : {}), ...(to === "released" ? { releaseReceiptHash: data.releaseReceiptHash } : {}) }); }
...
function repairChallengeCreated(p, d) { runtimeOnly(p); requireExactFields(d, ["challengeId", "executionRevision", "episodeId", "action", "subjectHash", "sessionId", "requestedAt", "expiresAt"], "repair challenge"); const episode = p.repairEpisodes.get(d.episodeId); const rejectSubject = hashCanonical({ goalId: p.goalId, executionRevision: p.executionRevision, episodeId: episode?.episodeId, conditionId: episode?.conditionId, findingIds: [...(episode?.findingIds || [])].sort() }); if (!episode || p.repairChallenges.has(d.challengeId) || d.executionRevision !== p.executionRevision || (d.action === "authorize_task" ? episode.status !== "active" : d.action === "reject" && ["active", "waiting_for_tasks", "reverifying"].includes(episode.status) && d.subjectHash === rejectSubject ? false : true) || !hash(d.subjectHash) || !d.sessionId || !Number.isFinite(d.requestedAt) || !Number.isFinite(d.expiresAt) || d.expiresAt <= d.requestedAt) throw new Error("invalid repair challenge"); p.repairChallenges.set(d.challengeId, { ...structuredClone(d), phase: "created", userEntryId: null }); }
function repairUserDecisionRecorded(p, d) { runtimeOnly(p); requireExactFields(d, ["challengeId", "sessionId", "userEntryId", "approved", "source", "recordedAt"], "repair user decision"); const c = p.repairChallenges.get(d.challengeId); if (!c || c.phase !== "created" || c.sessionId !== d.sessionId || !Number.isFinite(d.recordedAt) || d.recordedAt < c.requestedAt || d.recordedAt >= c.expiresAt || !d.userEntryId || typeof d.approved !== "boolean" || !["interactive", "rpc"].includes(d.source)) throw new Error("invalid repair user decision"); c.phase = d.approved ? "approved" : "rejected"; c.userEntryId = d.userEntryId; c.source = d.source; c.recordedAt = d.recordedAt; }
function repairCapabilityConsumed(p, d) { runtimeOnly(p); requireExactFields(d, ["nonceDigest", "consumedAt", "challengeId", "episodeId", "action", "subjectHash", "sessionId", "userEntryId"], "repair capability consume"); const c = p.repairChallenges.get(d.challengeId), episode = p.repairEpisodes.get(d.episodeId); const allowed = d.action === "authorize_task" ? episode?.status === "active" : d.action === "reject" && ["active", "waiting_for_tasks", "reverifying"].includes(episode?.status); if (!c || c.executionRevision !== p.executionRevision || !allowed || c.phase !== "approved" || !hash(d.nonceDigest) || !Number.isFinite(d.consumedAt) || c.recordedAt > d.consumedAt || d.consumedAt >= c.expiresAt || ["episodeId", "action", "subjectHash", "sessionId", "userEntryId"].some((key) => c[key] !== d[key])) throw new Error("invalid repair capability consume"); c.phase = "consumed"; c.nonceDigest = d.nonceDigest; c.consumedAt = d.consumedAt; }
