...
function repairChallengeCreated(p, d) { runtimeOnly(p); const fields = ["challengeId", "goalId", "executionRevision", "executionContractHash", "baseHead", "episodeId", "conditionId", "findingIds", "action", "subjectHash", "taskId", "taskDefHash", "sessionId", "requestedAt", "expiresAt", "challengeHash"]; requireExactFields(d, fields, "repair challenge"); const episode = p.repairEpisodes.get(d.episodeId); const rejectSubject = hashCanonical({ goalId: p.goalId, executionRevision: p.executionRevision, episodeId: episode?.episodeId, conditionId: episode?.conditionId, findingIds: [...(episode?.findingIds || [])].sort() }); const challengeHash = hashCanonical(Object.fromEntries(fields.filter((key) => key !== "challengeHash").map((key) => [key, d[key]])));
  if (!episode || p.repairChallenges.has(d.challengeId) || d.goalId !== p.goalId || d.executionRevision !== p.executionRevision || d.executionContractHash !== p.executionContractHash || d.baseHead !== p.runtimeBaseHead || d.conditionId !== episode.conditionId || !Array.isArray(d.findingIds) || JSON.stringify(d.findingIds) !== JSON.stringify([...episode.findingIds].sort()) || !hash(d.subjectHash) || !hash(d.challengeHash) || d.challengeHash !== challengeHash || !/^[a-f0-9]{40}$/.test(d.baseHead || "") || !d.sessionId || !Number.isFinite(d.requestedAt) || !Number.isFinite(d.expiresAt) || d.expiresAt <= d.requestedAt || (d.action === "authorize_task" ? episode.status !== "active" || !d.taskId || !hash(d.taskDefHash) : d.action !== "reject" || !["active", "waiting_for_tasks", "reverifying"].includes(episode.status) || d.subjectHash !== rejectSubject || d.taskId !== null || d.taskDefHash !== null)) throw new Error("invalid repair challenge"); p.repairChallenges.set(d.challengeId, { ...structuredClone(d), phase: "created", userEntryId: null }); }
function repairUserDecisionRecorded(p, d) { runtimeOnly(p); requireExactFields(d, ["challengeId", "sessionId", "userEntryId", "userEntryHash", "branchBindingHash", "userEntryOccurredAt", "choice", "approved", "source", "recordedAt", "decisionId"], "repair user decision"); const c = p.repairChallenges.get(d.challengeId); const decision = hashCanonical({ challengeId: d.challengeId, sessionId: d.sessionId, userEntryId: d.userEntryId, userEntryHash: d.userEntryHash, branchBindingHash: d.branchBindingHash, choice: d.choice, approved: d.approved, source: d.source, userEntryOccurredAt: d.userEntryOccurredAt, recordedAt: d.recordedAt }); if (!c || c.phase !== "created" || c.sessionId !== d.sessionId || [...p.repairChallenges.values()].some((x) => x.userEntryId === d.userEntryId) || !hash(d.userEntryHash) || !hash(d.branchBindingHash) || !hash(d.decisionId) || d.decisionId !== decision || !Number.isFinite(d.userEntryOccurredAt) || !Number.isFinite(d.recordedAt) || !(c.requestedAt < d.userEntryOccurredAt && d.userEntryOccurredAt <= d.recordedAt && d.recordedAt < c.expiresAt) || !d.userEntryId || !["approve", "reject"].includes(d.choice) || d.approved !== (d.choice === "approve") || !["interactive", "rpc"].includes(d.source)) throw new Error("invalid repair user decision"); c.phase = d.approved ? "approved" : "rejected"; Object.assign(c, structuredClone(d)); }
function repairCapabilityConsumed(p, d) { runtimeOnly(p); const fields = ["nonceDigest", "consumedAt", "challengeId", "episodeId", "action", "subjectHash", "sessionId", "userEntryId", "decisionId", "executionRevision", "executionContractHash", "baseHead", "taskId", "taskDefHash", "userEntryHash", "branchBindingHash"]; requireExactFields(d, fields, "repair capability consume"); const c = p.repairChallenges.get(d.challengeId), episode = p.repairEpisodes.get(d.episodeId); const allowed = (c?.action === "authorize_task" && episode?.status === "active") || (c?.action === "reject" && ["active", "waiting_for_tasks", "reverifying"].includes(episode?.status)); if (!c || !allowed || c.executionRevision !== p.executionRevision || c.phase !== "approved" || [...p.repairChallenges.values()].some((x) => x.nonceDigest === d.nonceDigest) || !hash(d.nonceDigest) || !Number.isFinite(d.consumedAt) || c.recordedAt > d.consumedAt || d.consumedAt >= c.expiresAt || ["episodeId", "action", "subjectHash", "sessionId", "userEntryId", "decisionId", "executionRevision", "executionContractHash", "baseHead", "taskId", "taskDefHash", "userEntryHash", "branchBindingHash"].some((key) => c[key] !== d[key])) throw new Error("invalid repair capability consume"); c.phase = "consumed"; c.nonceDigest = d.nonceDigest; c.consumedAt = d.consumedAt; }
