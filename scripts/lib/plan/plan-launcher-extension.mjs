...
export function createPlanLauncherExtension(pi, options = {}) {
    ...
    const timer = schedule(async () => {
      ...
      try {
        const results = await Promise.allSettled([
          ...
          rootIdentity().statusCaller(handle.planRunnerRunId),
        ]);
  ...
  pi.registerCommand("plan-status", { description: "Read Plan Runner status.", async handler(planId, ctx = {}) { const handle = await getHandle(planId, ctx); const runner = await rootIdentity().statusCaller(handle.planRunnerRunId); let raw; try { raw = await readFile(path.join(stateRoot, "var", "plan-runs", handle.planId, "status.json"), "utf8"); } catch (error) { if (error?.code === "ENOENT") return runner; throw error; } const plan = JSON.parse(raw); await forwardAttention(handle, plan, { failClosed: true }); return { runner, plan }; } });
  pi.registerCommand("plan-open", { description: "Open the Plan Runner artifact.", async handler(planId, ctx = {}) { const handle = await getHandle(planId, ctx); const status = await rootIdentity().statusCaller(handle.planRunnerRunId); const asyncDir = typeof status?.asyncDir === "string" && status.asyncDir ? status.asyncDir : typeof status?.details?.asyncDir === "string" && status.details.asyncDir ? status.details.asyncDir : handle.asyncDir; return { asyncDir, worktree: handle.worktree, status }; } });
  pi.registerCommand("plan-pause", { description: "Interrupt a running Plan Runner.", async handler(planId, ctx = {}) { const handle = await getHandle(planId, ctx); return rootIdentity().interruptCaller(handle.planRunnerRunId); } });
  pi.registerCommand("plan-cancel", { description: "Persist cancellation intent and stop the Plan Runner.", async handler(planId, ctx = {}) { const handle = await getHandle(planId, ctx); if (typeof options.recordCancelIntent === "function") await options.recordCancelIntent(handle); else await control.requestCancel({ planId: handle.planId, runId: handle.planRunnerRunId }); await rootIdentity().stopCaller(handle.planRunnerRunId); stopAttentionPoller(handle.planId); activeHandles.delete(planId); return `Plan ${planId} cancelled.`; } });
}
