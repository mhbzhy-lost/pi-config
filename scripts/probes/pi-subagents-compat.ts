...
export async function assertBrowserTranscriptCompatibility({
  ...
  const compat = await jiti.import(join(packageRoot, "src/compat/pi-subagents-0.62.ts"));
  for (const name of ["readFleetTranscript", "renderFleetTranscript", "getArtifactsDir"]) {
    if (typeof compat[name] !== "function") throw new Error(`pi-subagents browser compatibility missing ${name}`);
  }
}
...
export function evaluatePlanHarnessCompatibility(report) {
  ...
  if (report.version !== "0.62.0") failures.push(`unexpected pi-subagents version: ${report.version}`);
