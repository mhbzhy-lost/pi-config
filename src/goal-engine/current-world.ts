...
export function captureCurrentWorld({ repoRoot, adapterRegistry, environmentRegistry, fixtureRegistry, resourceRegistry = {}, runInventory = [], gitRunner = defaultGit } = {}) { try {
  ...
  return Object.freeze({ safe, repo: { root, head, branch, ...dirty, sequencer }, adapters, environments, fixtures, resources: resourceRows, activeRuns, capturedAt: new Date().toISOString() });
