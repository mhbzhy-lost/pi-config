...
function taskManifest(goalId, id, task, applicability) {
  ...
  out.workspaceProof = workspace?.schemaVersion === "managed-workspace.v1"
    ? { workspaceId: workspace.workspaceId, leaseId: workspace.leaseId, attempt: workspace.owner?.attempt, path: workspace.path, branchRef: workspace.branchRef, baseCommit: workspace.baseCommit, originRef: workspace.originRef, state: workspace.state, disposition: workspace.disposition }
