...
import { projectionStateHash } from "./store.ts";
import { coordinatorCriteria, executorCriteria } from "./task-definition.ts";
...
function evidenceSummary(task, settlement, proof) {
  ...
  const expectedCriteria = task?.acceptanceCriteria ?? acceptanceCriteria(task) ?? [];
  try {
...
function coordinatorPredicateSatisfied(task, predicate) {
  ...
  if (predicate === "workspace-integrated-released") return workspace?.schemaVersion === "managed-workspace.v1"
    ? workspace.state === "released" && workspace.disposition?.action === "integrate"
    : workspace?.phase === "disposed" && workspace.disposition === "integrated" && workspace.released === true;
...
function taskManifest(goalId, id, task, applicability) {
  ...
  out.settlementEvidence = evidenceSummary({ ...task, goalId, id, acceptanceCriteria: criteria ?? [] }, settlement, proof);
  out.settlementHash = sha(out.settlement);
  out.workspaceProof = workspace?.schemaVersion === "managed-workspace.v1"
    ? { schemaVersion: workspace.schemaVersion, workspaceId: workspace.workspaceId, leaseId: workspace.leaseId, attempt: workspace.owner?.attempt, path: workspace.path, branchRef: workspace.branchRef, baseCommit: workspace.baseCommit, originRef: workspace.originRef, state: workspace.state, disposition: workspace.disposition, serviceReceiptHash: task?.managedDisposition?.serviceReceiptHash === sha(workspace) ? task.managedDisposition.serviceReceiptHash : null, dispositionPhase: task?.managedDisposition?.phase ?? null, dispositionReceipt: task?.managedDisposition?.receipt ?? null }
    : workspace ? { attempt: workspace.attempt, path: workspace.path, branch: workspace.branch, baseCommit: workspace.baseCommit, originRef: workspace.originRef, requestedAction: workspace.requestedAction, strategy: workspace.strategy, executorHead: workspace.executorHead, originHeadBefore: workspace.originHeadBefore, legacyOriginRef: workspace.legacyOriginRef, originHead: workspace.originHead, phase: workspace.phase, disposition: workspace.disposition, released: workspace.released } : null;
  return out;
}
...
function deriveBlockers({ goalId, revision, contractHash, head: manifestHead, worldHash, tasks, conditions, debts: debt, stateHash: _stateHash, obligationStateHash: _obligationStateHash }) {
  ...
  for (const task of Array.isArray(tasks) ? tasks : []) { const id = task?.id; if (task?.applicability === "superseded") continue; const criteria = task?.acceptanceCriteria; if (!Array.isArray(criteria) || criteria.some(item => typeof item !== "string" || item.trim().length === 0) || new Set(criteria).size !== criteria.length) add(blockers, "TASK_DUAL_EVIDENCE_INVALID", id); if (!Array.isArray(task?.coordinatorCriteria) || task.coordinatorCriteria.some(criterion => !criterion || typeof criterion.id !== "string" || typeof criterion.predicate !== "string" || typeof criterion.satisfied !== "boolean")) add(blockers, "TASK_COORDINATOR_CRITERIA_INVALID", id); else for (const criterion of task.coordinatorCriteria) if (!criterion.satisfied) add(blockers, "TASK_COORDINATOR_PREDICATE_UNSATISFIED", `${id}:${criterion.id}`); if (task?.applicability === "reverify_required") add(blockers, "TASK_REVERIFY_REQUIRED", id); if (task?.status !== "accepted") add(blockers, "TASK_NOT_ACCEPTED", id); const b = task?.binding, p = task?.executorProofIdentity, s = task?.settlement, w = task?.workspaceProof;
    ...
    const canonicalWorkspace = w?.schemaVersion === "managed-workspace.v1";
    if (canonicalWorkspace) {
      const disposition = w.disposition, serviceDisposition = w.dispositionReceipt;
      if (!Number.isSafeInteger(w.attempt) || w.attempt !== b.attempt
          || w.path !== b.workspacePath || w.baseCommit !== b.headAtDispatch
          || w.state !== "released"
          || !isObject(disposition) || disposition.action !== "integrate" || !["cherry-pick", "merge"].includes(disposition.strategy)
          || w.dispositionPhase !== "receipt" || !hash(w.serviceReceiptHash)
          || !isObject(serviceDisposition) || serviceDisposition.released !== true
          || serviceDisposition.disposition?.action !== "integrate"
          || serviceDisposition.disposition?.strategy !== disposition.strategy) add(blockers, "TASK_WORKSPACE_UNCLOSED", id);
    } else {
    ...
    const e = task.settlementEvidence; try { const expected = { goalId, taskId: id, runId: p.runId, attempt: task.attempts, contractHash: task.contractHash, head: s.executorHead }; if (!e?.valid || e.schemaVersion !== "goal-engine.settlement-evidence.v1" || typeof e.path !== "string" || !hash(e.sha256) || e.path !== `acceptance-evidence/sha256/${e.sha256}.yaml` || e.mainSessionId !== p.rootSessionId || e.subagentFingerprint !== s.subagentFingerprint || e.mainFingerprint !== s.mainFingerprint) throw Error("evidence"); const sub = normalizeSettlementEvidence(e.subagent, { expectedIdentity: expected, expectedCriteria: criteria, outcome: "succeeded" }), main = normalizeSettlementEvidence(e.main, { expectedIdentity: expected, expectedCriteria: criteria, outcome: "succeeded" }); assertIndependentSettlementEvidence(sub, main); if (fingerprintSettlementEvidence(sub) !== e.subagentFingerprint || fingerprintSettlementEvidence(main) !== e.mainFingerprint) throw Error("fingerprint"); } catch { add(blockers, "TASK_DUAL_EVIDENCE_INVALID", id); }
  }
  ...
  for (const row of Array.isArray(debt?.workspaces) ? debt.workspaces : []) {
    if (row?.schemaVersion === "managed-workspace.v1") {
      if (row.state !== "released" || row.disposition?.action !== "integrate" || !["cherry-pick", "merge"].includes(row.disposition?.strategy)) add(blockers, "TASK_WORKSPACE_UNCLOSED", row?.id);
    } else if (row?.phase !== "disposed" || row?.disposition !== "integrated" || row?.released !== true) add(blockers, "TASK_WORKSPACE_UNCLOSED", row?.id);
  }
