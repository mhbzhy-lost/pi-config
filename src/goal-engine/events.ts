...
export function createProjection() {
  return {
    ...
    finalReview: null,
    consumedAmendmentNonceDigests: new Set(),
  };
}
...
export function applyEvent(projection, event, { replay = false } = {}) {
  ...
  switch (event.type) {
    ...
    case "task.dispatch_requested": taskDispatchRequested(next, event.data, event.schemaVersion); break;
...
function copyProjection(p) {
  return {
    ...
    consumedAmendmentNonceDigests: new Set(p.consumedAmendmentNonceDigests || []),
...
function evidenceInvalidated(p, data) { runtimeOnly(p); const condition = p.conditions.get(data.conditionId), amendment = p.pendingHumanDecision?.phase === "consumed"; if (amendment) { requireExactFields(data, ["conditionId", "revision", "priorEvidenceIds", "reason"], "amendment evidence invalidation"); if (!condition || data.revision !== p.pendingHumanDecision.newRevision || !Array.isArray(data.priorEvidenceIds) || JSON.stringify(data.priorEvidenceIds) !== JSON.stringify(condition.supportingEvidenceIds) || typeof data.reason !== "string" || !data.reason) throw new Error("invalid amendment evidence invalidation"); condition.supportingEvidenceIds = []; } else { requireExactFields(data, ["conditionId", "reason"], "evidence invalidation"); if (!condition) throw new Error("unknown condition"); } condition.status = "stale"; condition.invalidationReason = data.reason || null; }
...
function amendmentApproved(p, data) {
  runtimeOnly(p);
  const fields = ["proposalId", "proposalHash", "ownerSessionId", "userEntryId", "userEntryHash", "branchBindingHash", "choice", "approved", "source", "recordedAt", "decisionId"];
  requireExactFields(data, fields, "amendment approval");
  const pending = p.pendingHumanDecision, { decisionId, ...material } = data;
  if (!pending || pending.phase !== "proposed" || pending.proposalId !== data.proposalId || pending.proposalHash !== data.proposalHash || pending.ownerSessionId !== data.ownerSessionId || data.ownerSessionId !== ownerSessionId(p) || !data.userEntryId || !hash(data.userEntryHash) || !hash(data.branchBindingHash) || !hash(data.decisionId) || !["approve", "reject"].includes(data.choice) || data.approved !== (data.choice === "approve") || !["interactive", "rpc"].includes(data.source) || !canonicalIso(data.recordedAt) || data.decisionId !== hashCanonical(material)) throw new Error("invalid amendment approval");
...
function currentExecutionRevision(p) {
  return generationCapabilities(p.eventSchemaVersion).executionRevision ? p.executionRevision : 1;
}
...
function taskDispatchRequested(p, data, schemaVersion) {
  ...
  assertWorkspaceRedispatchable(task);
  if (!Number.isSafeInteger(attempt) || attempt !== task.attempts + 1) throw new Error("workspace request attempt mismatch");
  if (!hash(contractHash) || !gitHead(baseCommit)) throw new Error("workspace request contract or base commit is invalid");
  requireNonEmptyStrings({ workspaceId, originRoot, requestedCwd, originRef }, "workspace request");
  if (!isAbsolute(originRoot) || !isAbsolute(requestedCwd)) throw new Error("workspace request paths must be absolute");
  const expectedWorkspaceId = deterministicGoalWorkspaceId({
    goalId: p.goalId, taskId, attempt, executionRevision: currentExecutionRevision(p), contractHash, baseCommit,
  });
  if (workspaceId !== expectedWorkspaceId) throw new Error("workspace request id mismatch");
  task.status = "dispatch_requested";
...
function taskWorkspaceAllocated(p, data, schemaVersion) {
  ...
  if (workspace.state !== "active" || workspace.run !== null || workspace.workspaceId !== request.workspaceId
      ...
      || owner.attempt !== request.attempt || owner.executionRevision !== currentExecutionRevision(p)) {
    throw new Error("workspace allocation receipt does not match the current dispatch request");
  }
  task.workspace = structuredClone(workspace);
  task.status = "dispatched";
}
...
function requireWorkspace(task, attempt) {
  ...
  const workspaceAttempt = task.workspace.attempt ?? task.workspace.owner?.attempt;
  if (workspaceAttempt !== attempt) throw new Error("workspace attempt mismatch");
...
function goalCompleted(p, data, occurredAt, eventVersion) {
  ...
  if (p.eventSchemaVersion === RUNTIME_SCHEMA_VERSION) {
    ...
    if (data.verdict !== "COMPLETE" || !review || review.status !== "recorded" || !["none", "minor"].includes(review.severity)
      || data.reviewId !== review.reviewId || data.manifestHash !== review.manifestHash || data.stateHash !== review.stateHash || data.worldHash !== review.worldHash || data.head !== review.head || data.resultHash !== review.resultHash) throw new Error("invalid runtime final review completion");
  }
  const { verdict } = data;
