...
import { loadConfig } from "../src/compat/pi-subagents-0.62.ts";
import { browserTrustedRoots, readBrowserRunStatus, renderBrowserTranscript } from "../src/tui/browser-adapter.ts";
import { NativeChildConversationRenderer } from "../src/tui/native-conversation.ts";
import { SubagentSessionBrowserState } from "../src/tui/session-browser.ts";
import { parseSgrWheelDirection, ReadOnlyBrowserEditor, SubagentTranscriptViewport } from "../src/tui/session-viewport.ts";
import { layoutFooter } from "../src/tui/footer-layout.ts";
import { getTitleRegistry } from "../src/subagent-dispatch/title-registry.ts";
...
function selectorChild(child: { key: string; runId?: string; agent: string; state?: string; presentation?: string; label?: string }, selectedKey: string | undefined): string {
  const title = (child.runId ? getTitleRegistry().titleFor(child.runId) : undefined) ?? child.label;
...
function wrapSelectorItem(item: string, width: number): string {
  if (width <= 1) return item.startsWith("›") ? "›" : item.slice(0, 1);
  const prefixLength = item.startsWith("›") || item.startsWith("  ") ? 4 : 2;
  const prefix = item.slice(0, prefixLength);
  const content = item.slice(prefixLength).trimStart();
  const available = width - prefixLength;
  ...
  for (const word of content.split(/\s+/).filter(Boolean)) {
    if (visibleWidth(word) > available) {
      ...
      let remainder = word;
      while (visibleWidth(remainder) > available) {
        let cut = 0;
        let used = 0;
    ...
    if (line && visibleWidth(candidate) > available) { lines.push(line); line = word; }
    else line = candidate;
  }
  if (line) lines.push(line);
  return lines.map((value, index) => `${index === 0 ? prefix : " ".repeat(prefixLength)}${value}`).join("\n");
}
...
export function formatBrowserSelector(snapshot: ReturnType<SubagentSessionBrowserState["snapshot"]>, width: number): string {
  ...
  if (!childMode) return items.map((item) => wrapSelectorItem(item, Math.max(0, width))).join("\n");
  ...
  const wrappedItems = items.slice(start, end);
  if (!childMode && end < items.length) wrappedItems.push(...items.slice(end));
  return wrappedItems.map((item) => wrapSelectorItem(item, safeWidth)).join(childMode ? "  " : "\n");
}
...
export function createFooterComponent({ getCwd, getHome, getModel, getContextUsage, getThinkingLevel, getSnapshot, getViewportPosition, getExtensionStatuses, requestRender, theme }: any) {
  return { dispose() {}, invalidate() { requestRender(); }, render(width: number) {
    ...
    const selectorLines = formatBrowserSelector(snapshot, Math.max(1, width - visibleWidth(childRightLabel) - 1)).split("\n");
    const selectorFirstLine = selectorLines.shift() ?? "";
    const selectorThirdLine = snapshot.active ? selectorLines.join("  ") : selectorLines.shift() ?? "";
    const firstRows = [
      layoutFooter({ width, left: displayedCwd, right: contextLabel, visibleWidth, truncateToWidth }),
      layoutFooter({ width, left: selectorFirstLine, right: childRightLabel, visibleWidth, truncateToWidth }),
      layoutFooter({ width, left: selectorThirdLine || schedulerStatus, right: `thinking: ${thinking}`, visibleWidth, truncateToWidth }),
    ];
    return [...firstRows, ...(snapshot.active ? [] : selectorLines)].map((line) => theme.fg("dim", line));
  } };
}
  ...
  pi.on("session_start", (event: any, nextCtx) => {
    ...
    eventUnsubscribes = [
      pi.events.on("subagent:async-started", (event: any) => {
        if (owner !== ctx || epoch !== sessionEpoch) return;
        const eventTitle = typeof event?.title === "string" && event.title.trim() ? event.title : undefined;
        const fallbackTitle = eventTitle ? undefined : getTitleRegistry().started(event);
        browser.trackStarted(fallbackTitle ? { ...event, title: fallbackTitle } : event);
        invalidateNative(); persist(); refresh();
      }),
