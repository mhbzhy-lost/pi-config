...
Plan Runner handle 中的初始 runId 同时是稳定 logical caller。每次 revival 都创建新的 actual runId、token 和 grant；旧 actual identity 立即失效，但各 generation 继续写入同一 canonical Plan Runner session。Root 只在当前 actual generation 已有 official observed terminal proof，且存在 durable follow-up、queued push 或 live Executor completion debt时启动下一代。transport write、status、ACK 与模型文本都不能替代该 proof。
...
`test/plan-flat-runtime-harness.integration.mjs` 是并行、Attention、multi-generation continuation 和 Root shutdown 的真实拓扑门禁；`test/plan-amendment-harness.integration.mjs` 只补充 `plan.amended` 已提交而 current pointer 尚未写入时的同 Root crash/revival。旧 parallel Standalone Host Harness 与无调用方 shared RPC client 已删除。真实 Harness 必须使用 owned 空 `PI_CODING_AGENT_DIR`，并以 official terminal sidecar、canonical session、broker diagnostics、PID `ESRCH` 和 socket `ENOENT` 作为终态证据。
