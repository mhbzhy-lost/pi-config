...
- `origin/main` 的原四个配置提交已通过 `f490312` 纳入；执行期间新增的 `e8b7869` 通过 `9877644` 合入，`ca9ddd7` Skill 自动发现迁移通过 `a023e35` 合入。
- 远端配置冲突按既定边界解决：保留 DeepSeek 默认值和自定义 Provider 模型，同时继续移除退役 Todo package。
- 自动发现迁移暴露的 Doctor 聚合错误和外部全局 `plan-runner-dispatch` 复活风险分别在 `a023e35`、`6ad3d05` 修复；未修改对应外部 Skill，只在 Pi discovery 边界过滤退役名称。
...
对生产入口扫描确认 `plan_executor_supervisor`、`plan_run`、`plan_attention_reply`、`setup:plan-runtime`、`scripts/lib/plan`、`/plan-run` 均无命中。`plan-runner-dispatch` 唯一运行时提及是 Skill discovery 的精确退役 denylist；真实 Pi RPC 已验证不会暴露该 Skill。
...
- 复审完成后才到达的远端 Skill 自动发现提交另行经过真实 Pi RED、bug-first 和最小 GREEN，防止外部全局 Plan Skill 重新出现。
...
| using-goal-engine Skill/discovery | 26/26 通过 |
...
| `npm test` | 767 项：766 通过、1 失败 |
| 远端 Skill 自动发现、Doctor、shell、init 聚焦回归 | 60/60 通过 |
