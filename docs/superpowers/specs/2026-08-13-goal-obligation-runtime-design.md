...
Repair 的权威事件为 `repair.challenge_created`、`repair.user_decision_recorded`、`repair.capability_consumed` 和 `repair.task_linked`。challengeHash 是不含自身 ID/hash 的完整 canonical challenge body（goal/revision/contract、调用者显式提供的 Host CurrentWorld base head、episode/condition/有序 finding、action/subject、task 身份、session、时间）的 digest，challengeId 是其前缀；不得把 activation 的 runtimeBaseHead 当作 CurrentWorld。decision 与 consume 都必须携带并精确匹配 challengeHash，decisionId 也包含它；choice/approved parity、entry/branch hash 与时间边界不变。public capability 必须是无额外字段的 plain object，nonce 明文仅存在 S3 临时 capability，账本只保存 digest，跨 challenge 的 nonce digest 重用也拒绝。`authorize_task` 只能以 `goal.amended → capability_consumed → task_linked` 单一批次物化：Store 与 Reducer 均逐项核对 consume、challenge、canonical candidate 和新增 Task 的 taskId、taskDefHash、subject、episode、executionRevision；S1 使用纯 `buildRemediationTaskCandidate` 重建 candidate，S2 记录真实用户 entry，S3 消费 capability。三者均不得改变 exact-eight Goal ABI。
