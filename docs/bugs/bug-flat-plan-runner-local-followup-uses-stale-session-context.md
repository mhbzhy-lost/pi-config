...
stale-context 原文：`This extension ctx is stale after session replacement or reload. Do not use a captured pi or command ctx after ctx.newSession(), ctx.fork(), ctx.switchSession(), or ctx.reload().`。
同一现场已有 official terminal proof `state:"observed"`，且 run 为 resumable，却只产生 1 个
generation；artifact 没有第二个 run、grant、alias 或 resume。
...
official observed proof 驱动的 revival。诊断证据只能表明 stale 异常发生在 Broker 能够接收
`callerFollowUp` 并 resume 之前，不能据此臆断 Root broker 已接受 wake。Plan 停在可继续或需要
Gate 继续的中间状态，终态摘要不应被借机扩大成本轮修复范围。
...
4. 观察本地旧 context 报 `This extension ctx is stale after session replacement or reload. Do not use a
   captured pi or command ctx after ctx.newSession(), ctx.fork(), ctx.switchSession(), or ctx.reload().`；
   现场只有 generation 1，没有第二个 run、grant、alias 或 resume。
...
后以新的 runId/generation 代表同一逻辑 caller；旧 Pi context 不再是该 caller 的有效 context，
所以本地 follow-up 使用了 stale session context。artifact 没有第二 run/grant/alias/resume，且没有
Broker alias stale fence 或 alias validation 错误；阻断发生在 Broker 能够接收 `callerFollowUp` 并
resume 之前，不能认定 broker 已接受 wake。
...
- 真实 Harness：保留 `state:"observed"` official proof 后产生第二个 generation，且不存在
  ...
  after ctx.newSession(), ctx.fork(), ctx.switchSession(), or ctx.reload().` 错误。
