...
当前 Pi 会发现并展示 `goal_init`、`goal_status`、`goal_dispatch`、`goal_settle`、`goal_accept`、`goal_amend`、`goal_integrate` 七个工具，但真实调用 `goal_status` 时立即失败：

```text
definition.execute is not a function
```

失败发生在 Goal Engine 业务逻辑执行前；没有读取或写入 `.state/goal-engine/`。
...
1. 在当前仓库启动会加载 `pi/extensions/goal-engine.ts` 的 Pi 会话。
2. 调用只读工具 `goal_status`。
3. 观察工具层返回 `definition.execute is not a function`。
