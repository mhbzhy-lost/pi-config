...
`executor` requires `dispatch-ir.v1`, never a free-form task; never invent `delegate(...)`. Deadlines never waive. Verify cwd, relevant paths, and facts; unknowns are not `knownFacts`, placeholders. Exact top-level/nested shape; no extra fields; repo-relative POSIX `relevantFiles`/`writePaths`; supported enumeration; non-empty arrays; positive integer timeout; criteria-only acceptance. `tdd` reason forbidden; `existing-tests`/`docs-only` reason required. Optional top-level `modelTier` is `"luna"` or `"terra"`; omission defaults to requested Luna. `modelTier` selects the requested primary Luna or Terra model; configured executor `fallbackModels` may be attempted for retryable provider, auth, quota, rate-limit, or network failures. Run/status/artifact actual-model metadata is authoritative. Model tier is a coding resource choice within `dispatch-ir.v1`: generic dispatch must not bypass the typed coding contract to select an executor model.
