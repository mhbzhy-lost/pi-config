...
Completion/status is not terminal proof. Call `workspace_status(workspace_id)`, then `workspace_disposition(workspace_id, disposition, strategy?, action_token)`. Only official observed terminal proof permits destructive discard/integrate. `preserve` keeps it; `discard` releases clean workspace; `integrate` is coding-only after `writePaths` checks. Generic cannot integrate. No disposition: retain long-lived `awaiting-disposition`.
